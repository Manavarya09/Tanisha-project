...
export default function AssessmentForm() {
  ...
  const handleNext = () => {
    if (currentStep < totalSteps - 1) {
      ...
      // Save assessment data to localStorage and navigate to results
              ...
              <div key={question.question} className="space-y-4">
                ...
                <div className="space-y-2">
                  {[1,2,3,4,5].map((value) => (
                    <label
                      key={value}
                      className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${assessmentData.responses[question.question] === value ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}
                    >
                      <input
                        type="radio"
                        name={question.question}
                        value={value}
                        checked={assessmentData.responses[question.question] === value}
                        onChange={() => updateResponse(question.question, value)}
                        className="sr-only"
                      />
                      <div className={`w-4 h-4 rounded-full border-2 mr-3 ${assessmentData.responses[question.question] === value ? 'border-blue-500 bg-blue-500' : 'border-gray-300'}`}>
